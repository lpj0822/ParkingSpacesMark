...
QList<QString> DirProcess::getDirFileName(const QString &pathDir,const QString filterPostfix)
{
    ...
    if(dir->exists())
    {
        ...
        QList<QFileInfo> *fileInfo = new QList<QFileInfo>(dir->entryInfoList(filter, QDir::Files | QDir::Hidden | QDir::NoSymLinks));
        int fileCount = fileInfo->count();
        for(int loop = 0; loop < fileCount; loop++)
        {
            //qDebug()<<fileInfo->at(loop).absoluteFilePath();
...
void DirProcess::getDirAllFileName(const QString &pathDir, const QString filterPostfix, QList<QString> &allFileName)
{
    ...
    if(dir.exists())
    {
        QStringList filter;
        filter << filterPostfix;
        QFileInfoList fileInfo = dir.entryInfoList(filter, QDir::Files | QDir::Hidden |QDir::NoSymLinks);
        QFileInfoList dirInfo = dir.entryInfoList(QDir::Dirs | QDir::NoDotAndDotDot |QDir::NoSymLinks);
        int fileCount = fileInfo.count();
        int dirCount = dirInfo.count();
        for(int loop = 0; loop < fileCount; loop++)
        {
            allFileName.append(fileInfo.at(loop).absoluteFilePath());
        }
        for(int loop = 0; loop < dirCount; loop++)
        {
            getDirAllFileName(dirInfo.at(loop).absoluteFilePath(), filterPostfix, allFileName);
        }
    }
    else
    {
